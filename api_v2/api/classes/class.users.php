...
if(!isset($userInfo['Error']) && !is_array($userInfo['Error']) && count($userInfo['Error']) <=0){
				...
				if($userId > 0){
					$sqlQuery="update golf_users set is_new='0',user_name='".($email)."',display_name='".strtoupper($display_name)."',full_name='".strtoupper($fullName)."',alternate_email_id='".($email)."',password='".md5($password)."',activation_password='".($password)."',token='".$token."',device_token='".($device_token)."',device_os='".$device_os."',country_code='".$country_code."',registered_mobile_number='".$phone."',authorization_key= '".$authorization_key."',golf_course_id= '".$golf_course_id."' where user_id='".$userId."'";	
				...
				$uArr['display_name']=$fullName;	
				$uArr['full_name']=$fullName;	
				$uArr['user_name']=$email;	
				$uArr['self_handicap']=$handicap;	
	...
	function updateUserDeviceInfo($user_id,$device_token,$device_os,$logout=false)
	{
		$duser = array();
		 ...
		 $column = ($logout) ? "last_logout='".$today_date."',last_login='0000-00-00 00:00:00'" : "last_login='".$today_date."'";
		...
		$deviceIdValues  = $this->db->FetchSingleValue($device_exist);
		 ...
		 if($deviceIdValues>0){
			 ...
			 $user_data  = $this->db->FetchQuery($queryString);
				if(!$logout) {
					$queryString = "update golf_users set device_os='$device_os',device_token='{$device_token}' where user_id='".$user_id."'";
...
$email_id  = trim($email_id);
		...
		if(count($error) == 0) { 
			...
			if($this->db->FetchQuery($insert_Query)){
				...
				$message="Hi ".$user_data[0]["display_name"]." ,<br><br>Welcome to putt2gether - your own Live Golf Leaderboard!<br><br>As per your request for reset password your OTP is : <strong>".$otp."</strong> <br><br>
				We would love to hear from you regarding any feedback. Please drop us a mail at feedback@putt2gether.com!<br />
				<br />
				<div style='clear:both'></div>";
				sendmail($email_id,$user_data[0]["display_name"], $subject, $message);	
				$fdata = array();
				$fdata['status'] = '1';
...
$rowValues['thumb_url']=DISPLAY_PROFILE_PATH.'thumb/noimage.png';		
$rowValues['photo_url']=DISPLAY_PROFILE_PATH.'thumb/noimage.png';
}
$rowValues['golf_course_name']=(isset($rowValues['golf_course_name']) && $rowValues['golf_course_name'] !='')?$rowValues['golf_course_name']:'';
			...
			$chk_country='select country_id from country where upper(country_name)="'.strtoupper($rowValues['country']).'"';  
			$country_id  = $this->db->FetchSingleValue($chk_country);
			$rowValues['notifications_count'] = ($noti_count > 0) ? '1' : '0';
			$rowValues['country_id'] = ($country_id > 0) ? (string)$country_id : '0';
...
/*if($photo_url!="")
			{
				...
				}else
				{
					...
					$user_data['latest_event_id'] = 0;
					$user_data['format_id'] = 0;
					$new_user = 1;
				}
				
				$sqlchk4="select u.user_name,u.display_name,u.golf_course_id,u.registered_mobile_number,u.password,u.activation_password,p.self_handicap,p.photo_url,p.country from golf_users u left join user_profile p on u.user_id = p.user_id where u.user_id='{$userId}' limit 1";
				 $user_row = $this->db->FetchRow($sqlchk4);
				
				$new_user = 1;
				
				if((isset($user_row['user_name']) && trim($user_row['user_name'])!='') && (isset($user_row['display_name']) && trim($user_row['display_name'])!='') && (isset($user_row['registered_mobile_number']) && trim($user_row['registered_mobile_number'])!='') && (isset($user_row['password']) && trim($user_row['password'])!='') && (isset($user_row['self_handicap']) && trim($user_row['self_handicap'])!='') && (isset($user_row['country']) && trim($user_row['country'])!='')) {
					$new_user = 0;
				}
					
					$uvalue = $eveValues = array();
					$uvalue['user_id']=$userId;
					$uvalue['new_user']=$new_user;
		...
		elseif(!filter_var($email, FILTER_VALIDATE_EMAIL)) 
			...
			if($profile_img!="")
			{
				...
				if ($im !== false) {
					...
					$profile_img_path = ",photo_url='".$profile_img."'";
				}
				else {
					//echo 'correupted image';die;
				}
			...
			$sqlQuery="update ".TABLE_GOLF_USERS." set last_modified_date=now(),full_name='".strtoupper($full_name)."', display_name='".strtoupper($full_name)."', registered_mobile_number='".$contact_number."',golf_course_id='".$golf_course_id."', country_code='".$country_code."' ".$updcon." where user_id='".$user_id."'";	
			
			$this->db->FetchQuery($sqlQuery);
			
			$sqlQuery1="update user_profile set self_handicap='".$handicap."'{$profile_img_path}, country='".addslashes($country)."' ".$updcon1." where user_id='".$user_id."'";	
			$this->db->FetchQuery($sqlQuery1);
			$status = '1';
			$message = 'Profile Updated.';
			
			$sqlchk="select facebook_id from ".$this->golfUser_table." where user_id='".$user_id."'";
			$facebook_id  = $this->db->FetchSingleValue($sqlchk);
			
			$fdata['facebook_id'] = ($facebook_id>0) ? $facebook_id : 0;
			}
			}
			}else{
			$status = '0';
...
function ViewUserProfile($data){
		...
		if($user_id <= 0){
			...
			$fdata['message'] = 'Invalid User Id';
				...
				$score = $stats->getStatsPiChart(array("user_id"=>$user_id,"flag"=>1)); //print_r($score);die;
				...
				$fdata['data']['average_score'] = (isset($score['gross_score']) && $score['gross_score'] >0)?$score['gross_score']:0;
				}
			}
	...
	function logout($data) {
		...
		if($user_id>"0"){
			...
			$queryString = "select user_id from golf_users where user_id = '".trim($user_id)."'";
			$data2 = $this->db->FetchSingleValue($queryString);
			
			if($data2 > 0) {
				$this->updateUserDeviceInfo($user_id,$device_token,$device_os,true);	
