...
/*
					...
					if(count($event_invite_userids)>0) {
						foreach($event_invite_userids as $y=>$g){
							$this->add_event_invite_emails($g,$eventId,'','');
						}
					}
				}
...
//$emails = getEmailById($userId);
									...
									To view details download Putt2gether App from AppStore:.<br><br>Best Regards<br>Team Putt2gether!";
										...
										$this->add_event_invite_emails($userId,$eventId,$message,$subject);									
			...
			/*	if(count($temp_golf_course) > 0){
                ...
                if($event_is_team == 0)
                {
					...
					$i  = 1;
						...
						$this->add_event_invite_emails($particiantId['friend_id'],$eventId,'','');
...
$this->db->FetchQuery($sql);
		...
		function add_event_invite_emails($user_id,$event_id,$text,$subject) {
			$sqlQuery = "insert into event_invite_emails (";
			$sqlQuery .= " event_id,";
			$sqlQuery .= " player_id,";
			$sqlQuery .= " event_text,";
			$sqlQuery .= " event_subject,";
			$sqlQuery .= " is_send,";
			$sqlQuery .= " created_date";
			$sqlQuery .= " ) values (";
			$sqlQuery .= $event_id.", ";
			$sqlQuery .= " '".$user_id."',";
			$sqlQuery .= " '".htmlentities($text,ENT_QUOTES)."',";
			$sqlQuery .= " '".htmlentities($subject,ENT_QUOTES)."',";
			$sqlQuery .= " '0',";
			$sqlQuery .= " '".date('Y-m-d H:i:s')."'";
			$sqlQuery .= " )";
			$rowVal = $this->db->FetchQuery($sqlQuery);
		}
		...
		function randomHoleSelectionForDoublePeoria($eventId,$golf_course_id,$total_hole_num)
		{
		 ...
		 function startEventForPlayer($eventId,$player_id)
			...
			$queryString = " insert into event_score_calc(event_id,player_id,current_position) values(".$eventId.",".$player_id.",999)";
				...
				$queryString .= " handicap_value =". $handicap_value.",calculated_handicap=".$handicap_value.",";
				$queryString .= " handicap_value_3_4=".$rnd."";
				$queryString .= " where event_score_calc_id =".$event_score_calc_id;
				$queryResult = $this->db->FetchQuery($queryString);
				
				$queryString = "select no_of_putt_id from event_score_calc_no_of_putt where event_id = ";
				$queryString .= $eventId;
				$queryString .= " and player_id =";
				$queryString .= $player_id;
				$event_score_no_of_putt = $this->db->FetchSingleValue($queryString);
				
				if(!($event_score_no_of_putt > 0)) {
					...
					foreach($inlist as $k){
						$queryString = "INSERT INTO ".$k."(`event_id`, `player_id`) VALUES (".$eventId.",".$player_id.")";
						$this->db->FetchQuery($queryString);
					}
				}
		...
		if($eventId > 0 ) {
			...
			if(isset($eventdata) && $eventdata >0){
				...
				if(count($sql_str) > 0) {
					...
					elseif($event_is_singlescreen == 1) {
