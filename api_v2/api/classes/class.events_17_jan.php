...
$fdata = array();
			...
			$event_name = isset($eventInfoArray['event_name'])?addslashes($eventInfoArray['event_name']):"";
			$no_of_player = isset($eventInfoArray['no_of_player'])?$eventInfoArray['no_of_player']:"1";
			...
			if($event_name!="" && $event_start_date!="" && $event_start_time!="" && $event_golf_course_id!="" && $event_format_id!="" && $event_is_public!="" && $event_tee_id!="" &&  $event_admin_id!="" && $event_is_individual!="")
			{
							...
							if(count($grpsqlresult) > 0){
								foreach($grpsqlresult as $i=>$rowValues6){
									...
									if($allready_invite_chk_res ==0 ){
										...
										$sqlQuery1="insert into "._EVENT_PLAYER_LIST_TBL_." set event_id='".$eventId."',event_admin_id='".$event_admin_id."',player_id='".$userId."',is_new='1',is_accepted='".$is_accept."',accepted_by='".$accepted_by."',scorere_id='".$scorere_id."'";
									...
									To view details download Putt2gether App from AppStore:.<br><br>Best Regards<br>Team Putt2gether!";
										//	sendmail($rowValues6['user_name'], 'User', $subject, $message);
										}
										// sachin 10-8-15
									}
								}
...
function getEventInvitationList($data)

    {
        $fdata = array();
			...
			$queryString = "select e.player_id,e.is_accepted,eve.event_id,eve.event_name,eve.event_display_number,e.is_score_enterer,DATE(eve.event_start_date_time) as start_date,eve.event_start_time,eve.is_started,g.golf_course_name,g.golf_course_id,eve.format_id,eve.admin_id,u.full_name as admin,e.creation_date,e.is_submit_score,e.read_status,e.add_player_type from "._EVENT_PLAYER_LIST_TBL_." e left join "._EVENT_TBL_." eve ON eve.event_id=e.event_id left join ".TABLE_GOLF_USERS." u ON u.user_id=eve.admin_id left join "._GOLF_COURSE_TBL_." g ON g.golf_course_id=eve.golf_course_id WHERE eve.is_started in (0,1,3) and  e.player_id='".$user_id."'  and e.is_accepted!='2' and e.is_submit_score='0'  and DATE(eve.event_start_date_time)>= '".date('Y-m-d')."' order by start_date asc,eve.event_start_time asc, eve.event_id desc";
...
$con = ($request_to_participate <=0)?' AND p.player_id = '.$user_id.'':'';
		$queryString = "select e.*,g.golf_course_name,e.admin_id as event_admin_id,f.format_name,t.type,e.is_individual,p.is_score_enterer,p.scorere_id,p.is_submit_score,p.is_accepted,s.hole_number,s.no_of_holes_played,(select count(1) from event_player_list where event_id=".$eventId." and scorere_id=".$user_id.") as is_scorer from event_table e left join game_format f on f.format_id = e.format_id left join golf_course g ON g.golf_course_id = e.golf_course_id left join event_type t ON t.event_type_id = e.is_public LEFT JOIN event_player_list p ON p.event_id = e.event_id LEFT JOIN event_score_calc s ON s.event_id = e.event_id where e.event_id ='".$eventId."' ".$con." ";
...
}elseif($rowValues['no_of_holes_played'] == 0 && ($rowValues['is_admin'] == 1 && $rowValues['is_accepted'] == 1)){
...
$rowValues['is_singlescreen'] =$rowValues['is_singlescreen'];
				...
				$queryString = "select hole_number from event_is_spot_tbl where event_id ='".$eventId."' AND type = 1 ";
        			$closest_pin  = $this->db->FetchQuery($queryString);
					$queryString = "select hole_number from event_is_spot_tbl where event_id ='".$eventId."' AND type = 2";
        			$long_drive  = $this->db->FetchQuery($queryString);
					$queryString = "select hole_number from event_is_spot_tbl where event_id ='".$eventId."' AND type = 3 ";
...
function editEvent($data){
		...
		$event_name=isset($data['event_name'])?addslashes($data['event_name']):'';
		...
		$event_is_spot = isset($data['event_is_spot'])?$data['event_is_spot']:"0";
		$creation_date = date('Y-m-d H:i:s');
		$event_is_public = isset($data['event_is_public'])?$data['event_is_public']:"";
		$closest_pin = $long_drive = $straight_drive = 0;
		if($event_is_spot == '1') {
			$closest_pin = isset($data['closest_pin'])?$data['closest_pin']:"0";
			$long_drive = isset($data['long_drive'])?$data['long_drive']:"0";
			$straight_drive = isset($data['straight_drive'])?$data['straight_drive']:"0";
		}
		$ip_address = $_SERVER['REMOTE_ADDR'];
 ...
 //echo $eventId.'___'.$event_name.'___'.$event_start_date.'___'.$event_start_time.'___'.$event_golf_course_id.'___'.$event_format_id.'___'.$event_tee_id.'___'.$event_admin_id.'________'.$event_is_public; die;
		...
		$fdata = array();
		if($eventId > 0 && $event_name!="" && $event_start_date!="" && $event_start_time!="" && $event_golf_course_id >0 && $event_format_id >0 &&  $event_tee_id!="" &&  $event_admin_id> 0 && $event_is_public!= "")
			{
				...
				$ssql = "update "._EVENT_TBL_." set
				event_name='".$event_name."',
				golf_course_id='".$event_golf_course_id."',
				format_id='".$event_format_id."',
				admin_id='".$event_admin_id."',
				tee_id='".$event_tee_id."',
				event_start_date_time='".$event_start_date."',
				event_start_time='".$event_start_time."',
				created_by='".$event_admin_id."',
				creation_date='".$creation_date."',
				ip_address='".$ip_address."',
				is_spot='".$event_is_spot."',
				closest_pin='".$closest_pin."',
...
if($isExistPlayer > 0){
		$queryString = "Update "._EVENT_PLAYER_LIST_TBL_." SET is_accepted = '0' WHERE event_id = '".$event_id."' AND player_id = '".$user_id."' ";
				$rowValues  = $this->db->FetchQuery($queryString);
					...
					$queryString .= " event_id, event_admin_id, player_id,";
...
$rowValues  = $this->db->FetchRow($queryString12);
			...
			if(count($queryResult2) > 0)
			{
				...
				{
					$queryString3 = "update event_score_calc set  handicap_value =".($rowValues2['handicap_value'])." where player_id = ".$rowValues2['participant_id']." and event_score_calc.event_id = ".$eventId;
					$queryResult  = $this->db->FetchQuery($queryString3);

					$hadicap_3_4_value=round( ( $rowValues2['handicap_value'] * .75 ) , 0 );

					$queryString4 = "update event_score_calc set handicap_value_3_4=".$hadicap_3_4_value." where event_score_calc.player_id = ".$rowValues2['participant_id']." and event_score_calc.event_id = ".$eventId;
			...
			if(count($queryRes) > 0){
				foreach($queryRes as $i=>$rowVal){
					$calculated_handicap=($rowVal['handicap_value']-$minvalue);
...
if(isset($resdata) && $resdata>0){
		...
		function MyEventHistory($data){
			...
			if($user_id==""){
				...
				$current_date=date("Y-m-d");
				$queryString = "select e.event_id,e.format_id,(select count(player_id) from event_player_list where event_id = e.event_id and is_accepted=1) as no_of_player_accepted,(select count(player_id) from event_player_list where event_id = e.event_id) as no_of_player,e.total_hole_num,e.is_started,e.event_name,DATE(e.event_start_date_time) as event_start_date,e.event_start_time,e.golf_course_name from event_list_view e left join event_player_list epl on epl.event_id=e.event_id and epl.player_id='{$user_id}' left join golf_users u ON u.user_id=epl.player_id   WHERE 1=1 AND epl.is_submit_score='1' order by event_start_date desc";
				...
				if(count($queryResult) > 0) {
					foreach($queryResult as $i=>$row) {
						...
						$rowValues['no_of_player']= ($row['no_of_player']>4) ? "4+" : $row['no_of_player'];
					...
					$query1 = "select min(total_score) as gross_score,event_id from event_score_calc where player_id ='{$user_id}' and event_id in (".$mul_event.") group by event_id order by event_id asc limit 1";
					$rowValues1  = $this->db->FetchRow($query1);
					...
					$query3 = "select max(no_of_birdies) as no_of_birdies,event_id from event_score_calc where player_id ='{$user_id}' and event_id in (".$mul_event.") group by event_id order by event_id desc limit 1";
					$rowValues3 = $this->db->FetchRow($query3);
					...
					$query4 = "select max(no_of_pars) as no_of_pars,event_id from event_score_calc where player_id ='{$user_id}' and event_id in (".$mul_event.") group by event_id order by event_id desc limit 1";
					$rowValues4 = $this->db->FetchRow($query4);
