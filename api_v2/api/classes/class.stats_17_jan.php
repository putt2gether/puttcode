...
$event_list = '0';
		...
		$quet1 = "select s.event_id,e.last_modified_date as stime from event_score_calc s inner join event_table e on s.event_id = e.event_id inner join event_player_list p ON p.player_id = s.player_id and p.event_id=s.event_id where s.player_id= ".$player_id." and p.is_submit_score='1' order by stime DESC".$limit.""; 
...
if(isset($event_list) && is_array($event_list) && count($event_list)>0){
		foreach($event_list as $e=>$p){
		 ...
		 if($no_of_event > 1){
			...
			$query6  = "SELECT total_score as gross_score,last_modified_date as stime FROM event_score_calc s inner join event_table e on s.event_id = e.event_id where s.player_id= ".$player_id." and s.event_id in(".$event_list.") order by stime DESC"; 
			 $avg_gross_score  = $this->db->FetchQuery($query6);
			
$query7  = "SELECT s.par_total,s.total_score,e.last_modified_date as stime FROM event_score_calc s inner join event_table e on s.event_id = e.event_id where s.player_id= ".$player_id." and s.event_id in(".$event_list.") order by stime DESC"; 
			$avg_par_score  = $this->db->FetchQuery($query7);
			
$total_scores=$total_par=$total_gross_scores=$total_scores_all=0;//print_r($avg_par_score);die;
foreach($avg_par_score as $i=>$v){
                            $total_par+=$v['par_total'];
               $total_scores+=abs($v['par_total']-$v['total_score']); $total_scores_all+=$v['total_score'];
                        }
	foreach($avg_gross_score  as $i=>$p){
               $total_gross_scores+=($p['gross_score']);
         }
if($total_par > $total_scores_all){
$sign = '-';
}elseif($total_par < $total_scores_all){
$sign = '+';
}else{
$sign = '';
}
$events_no = count($avg_gross_score);
$no_of_event = ($events_no > $no_of_event)?$no_of_event:$events_no ;
$position_avg = round($total_scores/$no_of_event) ;
$score_avg = ($total_gross_scores/$no_of_event) ;
	...
	function getScoreStats($data){
		...
		if($admin_id > 0){
			$limit = ($no_of_event > 0)?' limit '.$no_of_event.'':'';
			$quet1 = "select s.event_id,e.no_of_player,e.is_singlescreen,e.total_hole_num,s.no_of_holes_played,e.last_modified_date as stime,p.scorere_id from event_score_calc s inner join event_table e on s.event_id = e.event_id inner join event_player_list p ON p.player_id = s.player_id and p.event_id=s.event_id where s.player_id= ".$admin_id ." and p.is_submit_score='1'  order by stime DESC ".$limit."";  
			$event_list  = $this->db->FetchQuery($quet1);
		...
		if(isset($event_list) && $event_list > 0){
			$query = 'SELECT concat(date(e.event_start_date_time)," ", e.event_start_time) as start_time ,p.event_id,c.total_score as gross_score,p.per_3_average as par_3s,p.per_4_average as par_4s,p.per_5_average as par_5s,p.gir_yes as gir_in,p.gir_no as gir_out,c.score_entry_1,c.score_entry_2,c.score_entry_3,c.score_entry_4,c.score_entry_5,c.score_entry_6,c.score_entry_7,c.score_entry_8,c.score_entry_9,c.score_entry_10,c.score_entry_11,c.score_entry_12,c.score_entry_13,c.score_entry_14,c.score_entry_15,c.score_entry_16,c.score_entry_17,c.score_entry_18 FROM event_score_calc c LEFT JOIN event_score_calc_no_of_putt p ON p.event_id = c.event_id left join event_table e ON e.event_id=p.event_id WHERE c.player_id = '.$admin_id.' AND p.player_id = '.$admin_id.' and c.event_id in('.$event_list.') ORDER BY start_time DESC';
			$allEvnt = $this->db->FetchQuery($query);
			
			if(count($allEvnt)>0){
$total_gross=$par3=$par4=$par5=$girout=$girin=$latesttotal_gross=$latestpar3=$latestpar4=$latestpar5=$latestgirout=$latestgirin=0;
			
				$latesttotal_gross+=($allEvnt[0]['gross_score']);
				$latestpar3+=($allEvnt[0]['par_3s']);
				$latestpar4+=($allEvnt[0]['par_4s']);
				$latestpar5+=($allEvnt[0]['par_5s']);
				//$latestgirout+=($allEvnt[0]['gir_out']);
				//$latestgirin+=($allEvnt[0]['gir_in']);

for($i=1;$i<=18;$i++){
       if($i <= 9){
          $latestgirin+= $allEvnt[0]['score_entry_'.$i];
       }else{
          $latestgirout+= $allEvnt[0]['score_entry_'.$i];
       }
...
for($i=1;$i<=18;$i++){
       if($i <= 9){
          ...
          $back+= $s['score_entry_'.$i];
						...
						//$girout+=($s['gir_out']);
						//$girin+=($s['gir_in']);
						$stats['avg_gross_score'] = ($total_gross >0 && $no_of_event>0)?round($total_gross/$no_of_event,2):'-';
						$stats['avg_par3s'] = ($par3 >0 && $no_of_event>0)?round($par3/$no_of_event,2):'-';
						$stats['avg_par4s'] = ($par4 >0 && $no_of_event>0)?round($par4/$no_of_event,2):'-';
						$stats['avg_par5s'] = ($par5 >0 && $no_of_event>0)?round($par5/$no_of_event,2):'-';
						$stats['avg_out'] = ($girout >0 && $no_of_event>0)?round($girout/$no_of_event,2):'-';
						$stats['avg_in'] = ($girin >0 && $no_of_event>0)?round($girin/$no_of_event,2):'-';
$stats['avg_out'] = ($back >0 && $no_of_event>0)?round($back/$no_of_event,2):'-';
$stats['avg_in'] = ($front >0 && $no_of_event>0)?round($front/$no_of_event,2):'-';

					}	
				}
				$stats['last_gross_score'] = ($latesttotal_gross >0)?round($latesttotal_gross/1,2):'-';
				$stats['last_par3s'] = ($latestpar3 >0)?round($latestpar3/1,2):'-';
				$stats['last_par4s'] = ($latestpar4 >0)?round($latestpar4/1,2):'-';
				$stats['last_par5s'] = ($latestpar5 >0)?round($latestpar5/1,2):'-';
				$stats['last_out'] = ($latestgirout >0)?round($latestgirout/1,2):'-';
				$stats['last_in'] = ($latestgirin >0)?round($latestgirin/1,2):'-';

$gscore_change = ($stats['avg_gross_score']>0) ? abs($stats['last_gross_score']-$stats['avg_gross_score'] )/$stats['avg_gross_score'] :0;
$gscore_change_color = ($stats['last_gross_score']>$stats['avg_gross_score'] ) ? 1 : (($stats['last_gross_score']==$stats['avg_gross_score'] ) ? 2  : 0) ;

$par3_change = ($stats['avg_par3s']>0) ? abs($stats['last_par3s']-$stats['avg_par3s'] )/$stats['avg_par3s'] :0;
$par3_change_color = ($stats['last_par3s']>$stats['avg_par3s'] ) ? 1 : (($stats['last_par3s']==$stats['avg_par3s'] ) ? 2  : 0) ;

$par4_change = ($stats['avg_par4s']>0) ? abs($stats['last_par4s']-$stats['avg_par4s'] )/$stats['avg_par4s'] :0;
$par4_change_color = ($stats['last_par4s']>$stats['avg_par4s'] ) ? 1 : (($stats['last_par4s']==$stats['avg_par4s'] ) ? 2  : 0) ;

$par5_change = ($stats['avg_par5s']>0) ? abs($stats['last_par5s']-$stats['avg_par5s'] )/$stats['avg_par5s'] :0;
$par5_change_color = ($stats['last_par5s']>$stats['avg_par5s'] ) ? 1 : (($stats['last_par5s']==$stats['avg_par5s'] ) ? 2  : 0) ;

$in_change = ($stats['avg_in']>0) ? abs($stats['last_in']-$stats['avg_in'] )/$stats['avg_in'] :0;
$in_change_color = ($stats['last_in']>$stats['avg_in'] ) ? 1 : (($stats['last_in']==$stats['avg_in'] ) ? 2  : 0) ;

$out_change = ($stats['avg_out']>0) ? abs($stats['last_out']-$stats['avg_out'] )/$stats['avg_out'] :0;
$out_change_color = ($stats['last_out']>$stats['avg_out'] ) ? 1 : (($stats['last_out']==$stats['avg_out'] ) ? 2  : 0) ;

				$stats['gscore_change'] =round($gscore_change,1);
				$stats['gscore_change_color'] =($gscore_change_color== 1)?'red':(($gscore_change_color== 2) ? "black" : 'green');

                                $stats['par3_change'] =round($par3_change,1);
				$stats['par3_change_color'] =($par3_change_color== 1)?'red':(($par3_change_color== 2) ? "black" : 'green');

                                $stats['par4_change'] =round($par4_change,1);
				$stats['par4_change_color'] =($par4_change_color== 1)?'red':(($par4_change_color== 2) ? "black" : 'green');

                                $stats['par5_change'] =round($par5_change,1);
				$stats['par5_change_color'] =($par5_change_color== 1)?'red':(($par5_change_color== 2) ? "black" : 'green');

                                $stats['in_change'] =round($in_change,1);
				$stats['in_change_color'] =($in_change_color== 1)?'red':(($in_change_color== 2) ? "black" : 'green');

                                $stats['out_change'] =round($out_change,1);
				$stats['out_change_color'] =($out_change_color== 1)?'red':(($out_change_color== 2) ? "black" : 'green');
	...
	function getGirPercentage($data){
		...
		if($admin_id > 0){
			$limit = ($no_of_event > 0)?' limit '.$no_of_event.'':'';
			$quet1 = "select e.is_singlescreen,e.no_of_player,e.total_hole_num,s.no_of_holes_played,s.event_id,e.last_modified_date as stime,p.scorere_id from event_score_calc s inner join event_table e on s.event_id = e.event_id inner join event_player_list p ON p.player_id = s.player_id and p.event_id=s.event_id where s.player_id= ".$admin_id ." and p.is_submit_score='1' order by stime DESC ".$limit."";  
			$event_list  = $this->db->FetchQuery($quet1);
...
//echo $total_played_hole; die;
					$yespercent = (isset($gir_yes) && $gir_yes >0)?$gir_yes/$total_played_hole:0;
					$nopercent =  (isset($gir_no) && $gir_no >0)?$gir_no/$total_played_hole:0;
...
$gir['hit'] = number_format( $yespercent * 100); 
					$gir['missed'] = number_format( $nopercent * 100); 
}
}
			}else{
				$gir['hit'] ='-'; 
	...
	function getFairwayPercentage($data){
		...
		if($admin_id > 0){
			$limit = ($no_of_event > 0)?' limit '.$no_of_event.'':'';
			$quet1 = "select s.event_id,e.is_singlescreen,e.no_of_player,e.total_hole_num,s.no_of_holes_played,e.last_modified_date as stime,p.scorere_id from event_score_calc s inner join event_table e on s.event_id = e.event_id inner join event_player_list p ON p.player_id = s.player_id and p.event_id=s.event_id where s.player_id= ".$admin_id ." and p.is_submit_score='1' order by stime DESC ".$limit."";  
			$event_list  = $this->db->FetchQuery($quet1);
			if(is_array($event_list) && count($event_list)>0){
				foreach($event_list as $e=>$p){
					if(($p['is_singlescreen'] == '2') || ($p['is_singlescreen'] == '1' && $p['scorere_id'] ==$admin_id)){
...
$hole = ($hole/$no_of_event);
$gir =($gir/$no_of_event);
$puttin = ($puttin/$no_of_event);
...
$putt['per_hole_avg'] = round($hole,2);
				$putt['per_gir_avg'] = round($gir,2) ;
				$putt['per_round_avg'] = round($puttin,2) ;
}
...
$scrm = (isset($scramble_yes) && $scramble_yes > 0)?round(($scramble_yes/$no_of_hole_played_yes)*100):0;
$san =(isset($sand_yes) && $sand_yes > 0)?round(($sand_yes/$no_of_hole_sand_yes)*100):0;
