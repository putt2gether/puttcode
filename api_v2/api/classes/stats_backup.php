...
$total_scores=$total_par=$total_gross_scores=$total_scores_all=0;
//print_r($avg_par_score);die;
foreach($avg_par_score as $i=>$v){
                            $total_par+=$v['par_total'];
               $total_scores+=abs($v['par_total']-$v['total_score']); 
$total_scores_all+=$v['total_score'];
                        }
...
if($no_of_event == 0){
$no_of_event =$events_no ;
}elseif($events_no > $no_of_event){
...
if($allEvnt[0]['par_'.$i] == 3){
	 $latestpar3+= $allEvnt[0]['score_entry_'.$i];
	$latest_total_hol_played_par3+= 1;
	}elseif($allEvnt[0]['par_'.$i] == 4){
	 $latestpar4+= $allEvnt[0]['score_entry_'.$i];
	$latest_total_hol_played_par4+= 1;
	}elseif($allEvnt[0]['par_'.$i] == 5){
	 $latestpar5+= $allEvnt[0]['score_entry_'.$i];
...
if($s['par_'.$i] == 3){
						...
						$stats['avg_par3s'] = ($par3 >0 && $no_of_event>0)?round($par3/$total_hol_played_par3,2):'-';
						$stats['avg_par4s'] = ($par4 >0 && $no_of_event>0)?round($par4/$total_hol_played_par4,2):'-';
						$stats['avg_par5s'] = ($par5 >0 && $no_of_event>0)?round($par5/$total_hol_played_par5,2):'-';
...
$stats['avg_in'] = ($front >0 && $no_of_event>0)?round($front/$no_of_event,2):'-';
				...
				$stats['last_gross_score'] = ($latesttotal_gross >0)?round($latesttotal_gross/1,2):'-';
		               $stats['last_par3s'] = ($latestpar3 >0)?round($latestpar3/$latest_total_hol_played_par3,2):'-';
				$stats['last_par4s'] = ($latestpar4 >0)?round($latestpar4/$latest_total_hol_played_par4,2):'-';
				$stats['last_par5s'] = ($latestpar5 >0)?round($latestpar5/$latest_total_hol_played_par5,2):'-';
