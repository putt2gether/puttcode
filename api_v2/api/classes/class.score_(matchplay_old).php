...
if(count($teamdata) > 0){
$golfdatta['is_team']=1;
    $golfdatta['teamdata']=$teamdata;
}
			/*	$queryString = " select t.player_id,t.handicap_value,t.score_entry_".$hole_number." as hole_num_".$hole_number.", p.no_of_putt_". */
$sqlp="SELECT t.player_id,g.full_name as player_name,t.format_id,t.no_of_holes_played FROM event_score_calc t left join golf_users g ON g.user_id=t.player_id where t.event_id='".$event_id."' group by t.player_id ";
            ...
            $is_team_game=false;$team_data=array();
                ...
                $currentScoreListArray['is_team'] =($is_team_game)?"1":"0";
                    if($is_team_game){
                       ...
                       $teamarray[]=array('current_standing'=>$standingdata['current']);
                       $team_data =$teamarray;
                    }
                }
                return $team_data;
					...
					foreach($score_a as $a=>$b) {
						...
						if($team_a_sum>0 && $team_b_sum>0){
							...
							elseif($team_b_sum == $team_a_sum) {
								...
								$hole_winner_class = $color_team_both;
								$winning_score = $pervious_winner_score;
								$current_winner_class = $color_bg_both;								
						...
						if($hole_winner_name == 'AS'){
							$winner_team_id=0;
						}else{
							if($is_team_game){
								$winner_team_id=($hole_winner_name=="TEAM A")?$uniqueteam[0]:$uniqueteam[1];
							}else{
								$winner_team_id=($hole_winner_name=="TEAM A")?$player_id[0]:$player_id[1];
							}
						}
...
t left join golf_users g ON g.user_id=t.player_id where t.event_id ='".$eventId."' ".$pl." order by t.player_id asc";
									...
									$total_front_9_postion=0;$total_back_9_postion=0;$total_postion=0;
									foreach($recordSetPlayerScore as $i=>$rowValues){
										$handi_counter=0;										
										for($ctr = $hole_start_from; $ctr <= $total_num_hole;  $ctr++){
											$handi_counter=$ctr;
