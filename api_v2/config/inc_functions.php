...
function sendmail($to,$toname,$subject, $message,$from='',$from_name='',$skip_content=false){	
	$contentMsg='';
	$contentMsg='<html>
<head>
<title>'.$subject.'</title>
...
<table id="Table_01" width="593" height="369" border="0" cellpadding="0" cellspacing="0" style="margin:20px auto;border:4px solid #0b5a97;">
	...
	<tr>
		<td colspan="3" style="font-family:arial;font-size:12px;padding:10px 0;padding-left:26px;color:#9b9e9f;line-height:18px;">'.$message.'</td>
		<td>
			<img src="'.__BASE_URI__.'newsletter/spacer.gif" width="1" height="98" alt="" style="float:left;margin:0;padding:0;outline:none;"></td>
	</tr>
	<tr>
...
<!-- End Save for Web Slices -->
	...
	if($skip_content == true) {$contentMsg = $message;}
	$obj=new Mail($to, $toname, $subject,$contentMsg);
	if($from==""){
	$obj->_from='feedback@putt2gether.com';
	$obj->_fromName='Team putt2gether';
...
return $data;
 ...
 function getEmailById($user_id){
		...
		global $database;
		$queryString = "Select user_name FROM golf_users WHERE user_id = ".$user_id." ";
		return $database->FetchSingleValue($queryString);
