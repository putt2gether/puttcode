...
//echo $query;die;
$event_data=$db->FetchQuery($query); //print_r($event_data);die;
?>
...
switch($list['is_started']){
	case 3:{ ?>
		...
		<a href="enter_score.php?id=<?php echo $list['event_id'];?>" target="_blank" class="btn btn-success">Enter Score</a>
...
<td><a href="banner-listing.php?event_id=<?php echo $list["event_id"]; ?>"  target="_blank">Banner</a></td>
<?php if(trim($_SESSION["a_user_level"])=="SA"): ?>
<td>
<div class="btn-group btn-group-sm btn-custom">
<?php if($list["is_active"]==1): ?>
<a class="btn btn-warning" id="status<?php  echo $list["event_id"]; ?>" event-id='<?php  echo $list["event_id"]; ?>' action='0' onclick="userStatus(this)"> Block </a>
<?php else: ?>
<a class="btn btn-success" id="status<?php  echo $list["event_id"]; ?>" event-id='<?php  echo $list["event_id"]; ?>' action='1' onclick="userStatus(this)"> Un-block </a>
<?php endif; ?>
</div>
</td>
<?php endif; ?>
