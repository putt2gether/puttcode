...
mysql_select_db(DB_NAME, $con) or die("no db connected");;
  ...
  $sqlQueryGet = "select pnu.push_notification_id as push_notification_id,pnu.event_id,";
			$sqlQueryGet .= "pnu.notification_text as notification_text, pnu.notification_code,apd.token as device_udid,apd.os  from push_notification_user_list pnu  join golf_user_app_devices apd on apd.user_id=pnu.user_id ";
		$sqlQueryGet .= " where  apd.os='1' and pnu.is_read = 0 and apd.status='1' order by pnu.push_notification_id";
    ...
    if (!$fp) {
		...
		$body['aps']['notifurl'] = 'https://putt2gether.com';
		$body['aps']['badge'] = 1;
		...
		while($row = mysql_fetch_array($result)) {
			...
			$body['aps']['alert'] =  $row["notification_text"].' '.$row["event_id"]; //$body['aps']['alert'] =  $row["notification_text"];
